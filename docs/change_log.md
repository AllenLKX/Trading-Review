...
- 设置页增加“请作者喝咖啡”入口，点击后展示微信支付二维码。
- 二维码使用不依赖登录会话的原始图片直链展示，保留手机浏览器长按保存到本地的系统能力。
- 弹窗补充“长按保存→打开微信→扫一扫→从本地图片识别”操作说明，并适配手机短屏滚动。
- 用户打开支持作者二维码时异步上报 `support_qr_opened`；日报可查看打开次数和去重用户数，不采集付款结果或金额。

## 2026-08-17

### 新手引导视觉与加载优化

- 全站 Logo 更新为新版扁平图，使用新的版本化资源路径，覆盖登录注册页、应用顶部、PWA 图标、加载页和离线页；Service Worker 缓存升级以避免旧图标残留。
- 三张新手引导图替换为新版 `941×1672` 竖版图片，以质量 `92` 的 WebP 保存，保留接近手机 2x 显示所需的原始宽度，总体积约 370KB。
- 新手引导增加“前一张”和“后一张”导航；后退只改变当前浏览位置，不回退账号已保存的最高进度。
- 导航按钮叠放在图片底部留白区，避免长图与按钮共同挤高页面；常见短屏和高屏继续完整显示图片。
- 第一张图在教程状态接口返回前开始预载，相邻图片随当前页提前解码；图片完整解码前显示加载层，完成后一次性淡入，避免 JPEG 下载时从上到下逐行显示。
- 永久使用教程增加“重新查看新手引导”，已完成教程的账号可以本地重播，不修改服务器完成状态。

### 账号级新手引导与永久教程

- 登录注册页主标语调整为“记录每一次判断，沉淀你的交易系统”，使用离散响应式字号并强制单行展示；`320`、`375`、`390` 和 `430` 像素宽度均无折行或横向溢出。
- 新账号及尚未完成教程的既有账号首次进入应用时，按顺序展示三张静态引导图；图片压缩为 900px 高质量 JPG，总体积约 730KB，并针对常见短屏和高屏平衡上下留白。
- 引导进度保存到账号的 `profiles.onboarding_step`，只允许向前推进；换设备或重新登录会从账号已保存的步骤继续。
- 第三张图后进入真实记录页并聚焦“新计划”按钮，到达该步骤即记为教程完成；用户可以直接创建计划，也可以稍后创建。
- 顶部设置按钮旁新增永久“使用教程”入口，以 HTML 弹层简要说明计划、操作、观察、独立复盘和周期分析的使用顺序。
- 认证回归增加教程初始状态、进度推进、禁止倒退、完成状态和重新登录持久化验证。

### 数据库迁移失败保护

- `apply-schema.sh` 增加 PostgreSQL `ON_ERROR_STOP`，数据库权限或 SQL 错误会立即终止发布，不再在迁移失败后误报成功。

### 产品品牌展示更新

- 产品对外展示名称统一改为“交易笔记本”，覆盖应用顶部栏、登录注册页、浏览器标题、PWA 安装信息、离线页、导出文件名和项目文档。
- 使用用户提供的新图作为统一品牌 Logo，并生成轻量的 512px PNG 资源；网页、主屏幕安装图标和离线页共用同一资源。
- 新 Logo 加入认证中间件的公开静态资源白名单，确保未登录页面、图片优化请求和离线缓存都能正常读取。
- 登录和注册页新增主标语“记录每一次判断，沉淀你的交易系统”及辅助标语“复盘交易 · 优化决策 · 构建长期有效的方法”。
- 域名、部署目录、数据库结构、接口路径和既有本地存储 key 保持不变，避免品牌更新影响已有账号、登录态和数据兼容性。
...
- 交易笔记本已部署到 `/var/www/rationaltrade`，生产构建 commit 为 `a81d2a4`。
- PM2 进程已在线并保存开机恢复配置。
