...
- 新增仅供管理员在服务器执行的幂等演示数据脚本，以固定前缀写入三条跨月计划、十三条操作和六条独立复盘。
- 脚本通过目标账号的十分钟临时会话调用现有审计接口，要求得到真实 DeepSeek 结果后再归档，并在结束时立即撤销会话。
- 支持预览、写入和按固定前缀清理，不修改账号密码或目标账号其他记录。
- 演示内容属于合成样例，只用于展示长期记录与行为审计能力，不代表真实交易业绩。

### 分享来源归因

- 分享链接新增 `?adtag=渠道名` 约定，用于区分微信、小红书、朋友转发等外部传播来源。
- 客户端按最近一次非直接来源持久保存合法 `adtag`；未带参数的后续访问不会清除来源，新标签会覆盖旧标签。
- 未登录入口跳转登录页时保留 `adtag`，页面曝光、注册、登录成功和登录失败事件都携带来源标签。
- `adtag` 仅允许字母、数字、中文、点、下划线和连字符，最长 64 个字符；不保存完整分享 URL 或其他查询参数。
- 按日运营报告新增 `trafficSources`，直接输出各来源 PV、UV、注册数和成功登录数。
- 认证与运营回归新增入口跳转不丢标签、认证事件归因和来源汇总验证。

### 移动 Web 发布候选兼容性收尾

- 网页继续作为当前唯一交付物，本轮不创建 Capacitor、Android 或 iOS 原生壳。
- 移除 `maximum-scale=1`，保留用户双指缩放；所有移动端表单在窄屏使用至少 16px 字号，避免 iOS Safari 聚焦输入框时自动放大。
- 顶部栏、底部导航、截图固定操作栏、Toast、登录页和离线页统一计入 `safe-area-inset-*`，适配刘海屏和 Home Indicator。
- 新增在线状态监听：已打开页面断网时明确提示，保存失败继续保留用户已填写内容；Service Worker 仍只缓存离线提示页，不缓存业务数据。
- 离线页支持日间/黑夜偏好，使用 `100dvh` 并保留安全区；Service Worker 缓存版本升级，确保已安装 PWA 获取新离线页。
- 首屏不再在云端数据返回前展示示例计划，断网时只显示该浏览器最近一次成功读取的缓存；无缓存则显示真实空状态。
- 历史审计不再向正式用户混入示例归档，空账号仅展示真实空状态。
- 常见移动视口 `375x667`、`390x844`、`360x800`、`412x915` 已完成无横向溢出、固定栏位置和表单字号检查。

### 日间与黑夜显示模式

- 设置面板新增日间/黑夜切换，默认保留原黑夜模式，选择按浏览器持久保存并在 React 加载前恢复，避免刷新闪屏。
- Tailwind 硬编码颜色改为 CSS 语义变量，卡片、表单、正文、风险提示、成功提示和审计页面共用两套主题，不复制组件。
- 日间模式采用冷白灰背景、白色内容面和深色正文；390px 记录页与历史审计页无横向溢出，弱化文字和状态色完成对比度检查。
- Phase 1 本地回归通过账号会话与隔离、登录页漏斗、计划/操作/复盘 CRUD、截图事务与删除确认边界；合成数据均已清理。

### 真实 AI 截图补账与持久登录

- 新增隔离的真实 AI 全链路回归脚本，覆盖 Kimi 识图、人工基准比对、DeepSeek 结构化、事务归档、周期分析、分析归档和运营事件，并在结束后清理全部合成数据。
- 真实截图复核中，Kimi 正确识别“半导材料”，并纠正了人工基准误写为“半导体材料”的问题。
- 腾讯云生产全链路通过：Kimi + DeepSeek 正确得到 3 笔成交，截图识别约 39.5 秒、事务归档约 70 毫秒、DeepSeek 周期分析约 2.4 秒、分析归档约 25 毫秒，合成账号与事件已清理。
- 截图识别增加累计等待时间；超时或断网后在内存中保留刚选择的图片，可手动一键重试且无需重新选图，不进行可能重复计费的自动重试。
- 延迟诊断确认服务器内部健康接口约 14 毫秒、数据库连接查询约 100 毫秒；本机到新加坡首次 HTTPS 约 0.76 秒，复用连接后约 0.16 秒，普通请求的主要额外延迟来自地域网络，长截图等待主要来自 Kimi 推理。
- 本地无样式问题确认为开发服务运行期间执行 `next build` 导致 `.next` 开发 CSS 失效；重建开发缓存并重启后 CSS 恢复，现有 `verify-local.sh` 会在 3000 端口被占用时阻止再次并行构建。
- 新增 `POST /api/recognitions/archive`，将用户确认的多笔操作和自动新建计划在单个 PostgreSQL 事务中归档，任意一笔失败时整批回滚。
- 截图归档请求保留客户端 ID，相同请求重放时更新原记录而不创建重复操作；跨用户或跨计划 ID 冲突时整批拒绝。
- 前端从逐计划、逐操作串行保存切换为单次事务请求，成功后用服务端返回的完整计划更新缓存并保留 Toast。
- `pnpm verify:database` 新增混合挂靠既有/新计划、幂等重放、ID 冲突零残留和合成数据清理回归。
- 数据库回归支持命令级 `TEST_AUTH_COOKIE`，用于生产发布时的短期一次性会话，不需要 owner 真实密码。
...
- 日报新增登录页漏斗，按匿名访客去重统计登录页 UV、已注册或登录 UV、截至查询时仍未进入产品的 UV 与流失率。
- 登录和注册事件携带页面曝光使用的同一随机匿名 ID，用于建立转化关联；不采集 IP、邮箱内容或设备指纹。
- 漏斗设置明确采集起点，避免发布前无法关联的登录事件被误算为流失；首日标记为部分日数据。
...
- 账号代码与三张新表先以 Basic Auth 安全发布，owner 在服务器终端完成绑定后再切换 session，迁移期间历史数据始终保持 8 个计划、10 条操作、6 条复盘。
- owner 初始化后已切换 session 模式；修复 Nginx 反向代理下未登录跳转误用内部 `localhost:3000` 的问题，并让生产冒烟脚本分别验证 Basic 与 session 认证边界。
- 正式环境注册、登录、退出、会话撤销和新账号空数据隔离通过；一次性 owner 会话回读为 8 个计划、10 条操作、6 条复盘、3 条审计归档，测试会话与合成账号均已清理。
- 生产页面曝光上报得到 PV 1、UV 1；真实 DeepSeek 调用上报得到 1 次调用、362 输入 token、144 输出 token、506 总 token，未创建测试归档。
- 右上角恢复为设置入口，账号面板实时显示当前邮箱并提供退出登录；退出成功后返回登录页，读取或退出失败时保留当前页面并显示错误。
