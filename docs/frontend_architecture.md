...
引导图使用原始 941px 宽的高质量 WebP，并绕过运行时图片转码。第一张与相邻页由浏览器提前下载、完整解码后再整体淡入；前后导航的本地浏览位置与服务器只增不减的最高进度分离。底部导航叠放在图片预留空白区域，已完成账号可从永久教程入口本地重播。
...
- `lib/server/kimi-vision.ts`：Kimi K3 原图转写、超时和输出校验。
- `lib/server/vision-prompts.ts`：读取 Kimi 转写 Prompt 和版本。
- `lib/server/deepseek-recognition.ts`：转写文本的交易结构化和严格输出校验。
...
- 不提供投资建议
- AI 结果必须可解释、可确认
- 不限制页面缩放，移动端表单字号不得低于 16px，避免 iOS Safari 自动放大
- 顶部栏、底部导航、固定 CTA、Toast 和认证页必须计入系统安全区
- 正式数据未返回前不得展示示例业务数据；失败时只允许回退到该用户最近一次成功缓存
- 已打开页面断网时给出全局提示，所有失败保存继续保留表单输入
