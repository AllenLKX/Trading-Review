...
- H5 已增加显式云端自动保存模式，修改按顺序写入现有 CRUD API，失败保留本地数据并可重试。
...
- H5 继续以本地工作副本为写入源，云端作为明确确认后的 PostgreSQL 备份。
- 本地记录最近一次成功同步的内容指纹、时间和方向。
- 上传前读取云端并比较同步基线，云端变化时阻止静默覆盖。
- 冲突可选择下载云端替换本地，或合并后上传。
- 合并保留云端独有记录，同 ID 内容以本地为准。
- 状态读取、预览和上传失败提供重试入口。

当前限制：

- 批量导入采用 upsert，不把“本地缺少某条记录”解释为云端删除。
- 删除动作仍需后续实时云端模式通过明确 DELETE 请求同步。
- 当前不是后台自动同步，也不是多设备实时协作。
- 当前云端自动保存主要解决单设备、单用户的可靠写入；正式多设备并发仍需版本号或 ETag。
