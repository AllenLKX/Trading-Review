...
export async function generateDeepSeekAudit(aiRequest: AuditAiRequest): Promise<DeepSeekAuditResult> {
  ...
  try {
    const response = await fetch(`${baseUrl}/chat/completions`, {
      ...
      body: JSON.stringify({
        model,
        messages: buildMessages(aiRequest),
...
function buildMessages(aiRequest: AuditAiRequest) {
  const outputExample = {
    summary: "概括本周期的行为与复盘模式，不评价市场方向。",
    signalLabel: "简短状态标签",
    signalLevel: "stable",
    findings: ["基于输入记录的行为发现"],
    reviewQuestions: ["帮助用户核对原计划与实际执行的问题？"]
  };
  ...
  return [
    {
      ...
      content: [
        ...
        `JSON 输出示例：${JSON.stringify(outputExample)}`
      ].join("\n")
    },
    {
      role: "user",
      content: `请对以下 json 审计输入做行为复盘，并按约定 JSON 格式一次性输出：\n${JSON.stringify(aiRequest)}`
    }
  ];
}
