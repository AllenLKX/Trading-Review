...
- 创建和编辑交易计划
- 首次登录按账号保存的新手引导，以及顶部永久使用教程
- 在计划下记录买入、卖出、观察
- 在计划下记录独立复盘
- 编辑和删除计划时间线里的操作与复盘
- Kimi K3 识图 + DeepSeek 结构化的截图补账，逐条确认后批量归档
- 近 30 天审计摘要、复盘追问和 AI 输入摘要
- 用户主动触发的 DeepSeek 周期行为审计，失败时回退本地规则
- 审计快照归档、删除、清空
- 本地 JSON 导入导出，包含计划和审计归档
- AI 归档记录模型、Prompt 版本与生成/回退状态
- PWA manifest、主屏幕图标、安全区适配和不缓存交易数据的离线兜底
- 设置中切换并持久保存日间/黑夜显示模式
- 邮箱密码注册登录、可撤销服务端会话与按用户数据隔离
- 分享链接 `?adtag=渠道名` 的来源归因，以及按日期查询各来源 PV、UV、注册和登录
- 按日期查询整体 PV、UV、登录页流失、业务流水和 AI token 用量
...
当前 Backend M1 已部署，PostgreSQL 是正式数据源，localStorage 只作为最近一次成功读取的缓存。正式入口 `https://rationaltrade.cn` 已启用免费 HTTPS 和自动续期。
...
```bash
./scripts/configure-vision-ai.sh .env.local
```
...
- 记录页：创建计划，添加操作和复盘
- 记录页：进入截图补账，上传真实交易截图，确认识别结果后批量归档
- 历史页：查看计划详情，编辑或删除时间线记录
- 历史页：刷新审计，归档审计快照，再展开历史归档
- 历史页：导出 JSON，确认包含 `auditReports`
- 设置：切换日间/黑夜后刷新，确认主题保持
- 移动端：检查 `375x667`、`390x844`、`360x800`、`412x915`，确认无横向滚动且底栏不遮挡内容
- 断网：已打开页面显示离线提示，保存失败后表单内容不被清空；新导航进入静态离线页

完整的网页发布候选检查见 `docs/web_release_checklist.md`。
