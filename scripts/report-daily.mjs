  ...
  const [summary, eventCounts, activity] = await Promise.all([
    pool.query(
      `select
         ...
         count(*) filter (where event_name = 'ai_audit_generated')::int as ai_calls,
         coalesce(sum((metadata->>'promptTokens')::int) filter (where event_name = 'ai_audit_generated'), 0)::int as prompt_tokens,
         coalesce(sum((metadata->>'completionTokens')::int) filter (where event_name = 'ai_audit_generated'), 0)::int as completion_tokens,
         coalesce(sum((metadata->>'totalTokens')::int) filter (where event_name = 'ai_audit_generated'), 0)::int as total_tokens
